    ...
    internal class Day2
    {
        public static int[] DivePart1()
        {
            ...
            foreach (var line in data)
            {
                ...
                {
                    temp = line.Replace("forward ", string.Empty);
                    position[0] += int.Parse(temp);
                }

                if(line.Contains("down"))
                {
                    temp = line.Replace("down ", string.Empty);
                    position[1] += int.Parse(temp);
                }

                if(line.Contains("up"))
                {
                    temp = line.Replace("up ", string.Empty);
                    position[1] -= int.Parse(temp);
                }
            }
